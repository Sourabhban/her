...
/* ==========================================================================
   0. BULLETPROOF BACKGROUND VIDEO AUTOPLAY ENGINE
   ========================================================================== */
function initBackgroundVideo() {
  ...
  video.volume     = 0;      // extra safety — fully silent
  ...
  const tryPlay = () => {
    ...
    if (p !== undefined) {
      p.catch(() => {
        // If autoplay blocked, play on FIRST interaction of any kind
        const events = ['click', 'touchstart', 'touchend', 'mousemove', 'keydown', 'scroll', 'pointerdown'];
        const unlock = () => {
          video.play().catch(() => {});
  ...
  // Also retry once the video metadata is loaded (fixes some mobile browsers)
